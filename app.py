...
def main():
    ...
    with tab2:
        ...
        # ---------------- TAB 3 ----------------
    with tab3:
        st.subheader("Medical Podcast Sentiment Analysis")
        ...
        if not segments:
            ...
            with right_col:
                ...
                # ---------------- TAB 4 ----------------
    with tab4:
        st.subheader(f"Keyword Cloud for: {base_name}")

        if not segments:
            st.info("Please load an audio file to generate the keyword cloud.")
        else:
        # -----------------------------
        # Aggregate text from selected audio
        # -----------------------------
        ...
        # -----------------------------
        # Condition: medical OR fallback
        # -----------------------------
            if len(medical_freq) >= MIN_MEDICAL_TERMS:
                mode = "medical"
                final_freq = dict(medical_freq.most_common(20))
            else:
                mode = "general"
                final_freq = dict(Counter(words).most_common(20))
            ...
            # LEFT: Explanation + keyword list
                with left_col:
                    if mode == "medical":
                        st.markdown("###  Medical Keyword Cloud")
                        st.markdown("""
                        **How to read this cloud:**
                        -  **Bigger words** → Medical terms mentioned more frequently  
                        -  **Smaller words** → Less frequent medical terms  
                        - Only **medical-related terminology** is shown
                        """)
                    else:
                        st.markdown("###  General Keyword Cloud ")
                        st.markdown("""
                        **How to read this cloud:**
                        -  **Bigger words** → Words mentioned more frequently  
                        -  **Smaller words** → Less frequent words  
                        """)
